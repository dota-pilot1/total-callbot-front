...
export default function CharacterChatbotMobilePage() {
  ...
  // 캐릭터 상태 디버깅
  console.log("🎭 [CharacterChatbotMobilePage] 현재 캐릭터 상태:");
  console.log("  - zustand personaCharacter:", personaCharacter);
  console.log("  - zustand personaGender:", personaGender);
  console.log("  - actualPersonaCharacter:", actualPersonaCharacter);
  console.log("  - selectedVoice (from hook):", selectedVoice);
  ...
  return (
    <div className="h-screen bg-white flex flex-col">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-1">
              ...
              <Button
                ...
                onClick={() => {
                  console.log(
                    "Logout button clicked in CharacterChatbotMobilePage",
                  );
                  logout();
                }}
                title="로그아웃"
      ...
      <MobileCharacterDialog
        ...
        onConfirm={(v) => {
          console.log("🎭 [캐릭터 변경] 사용자가 새 캐릭터 선택:", v);

          // 캐릭터 설정 적용 (훅에서 처리) - 완료 콜백과 함께
          applyCharacterSettings(v, async () => {
            console.log("🎭 [캐릭터 변경] 설정 적용 완료, 음성 연결 처리 시작");

            if (voiceConn && isRecording) {
              console.log(
                "🎭 [캐릭터 변경] 음성 연결 중 - 페르소나 실시간 업데이트",
              );

              // 먼저 실시간 페르소나 업데이트 시도
              updatePersona();

              // 페르소나 업데이트 후 추가적인 안정성을 위해 연결 재시작
              console.log("🎭 [캐릭터 변경] 안정성을 위한 음성 연결 재시작");
              stopVoice();

              // 충분한 지연 시간으로 완전한 연결 해제 보장
              setTimeout(async () => {
                try {
                  setVoiceEnabled(true);
                  await startVoice();
                  console.log("🎭 [캐릭터 변경] 음성 연결 재시작 완료");
                } catch (error) {
                  console.error(
                    "🎭 [캐릭터 변경] 음성 연결 재시작 실패:",
                    error,
