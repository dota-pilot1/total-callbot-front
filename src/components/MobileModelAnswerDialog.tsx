...
import { examApi, type SampleAnswer } from '../features/exam/api/exam';
...
export default function MobileModelAnswerDialog({ open, onClose, question, topic, level }: MobileModelAnswerDialogProps) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setAnswers([]);
      try {
        const resp = await examApi.getSampleAnswers({ question, topic, level, count: 2 });
        if (!cancelled) setAnswers(resp.samples || []);
      } catch (e: any) {
        if (!cancelled) setError(e?.message || '모범 답변을 불러오지 못했습니다.');
      } finally {
            ...
            <div ref={scrollRef} className="px-4 py-3 h-[70vh] overflow-y-auto">
              ...
              <div className="mb-3 flex justify-start">
                <div className="max-w-[85%] bg-indigo-50 text-indigo-900 border border-indigo-200 rounded-lg px-3 py-2 whitespace-pre-wrap">
                  <div className="text-xs font-semibold text-indigo-700 mb-1">질문</div>
                  {question}
                </div>
              </div>
              ...
              {answers.map((a, idx) => (
                <div key={idx} className="mb-3 flex justify-end">
                  <div className="max-w-[85%] bg-white border border-gray-200 rounded-lg px-3 py-2 whitespace-pre-wrap">
                    <div className="text-xs font-semibold text-gray-600 mb-1">{a.title || `Ex${idx + 1}`}</div>
                    {a.text}
                  </div>
                </div>
              ))}
