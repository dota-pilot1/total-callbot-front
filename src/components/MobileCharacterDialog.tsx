...
interface MobileCharacterDialogProps {
  open: boolean;
  onClose: () => void;
  value?: { characterId: string; scenarioId?: string; gender: GenderOption; voice?: 'verse'|'alloy'|'sage' };
  onConfirm: (v: { characterId: string; scenarioId?: string; gender: GenderOption; voice: 'verse'|'alloy'|'sage' }) => void;
}

// 캐릭터 9개 (동화 3, 영화 3, 유명 인사 3)
const CHARACTERS: CharacterOption[] = CHARACTER_LIST.map(c => ({ id: c.id, name: c.name, emoji: c.emoji }));

// 극적인 장면 5개
const SCENARIOS: ScenarioOption[] = [
  { id: 'final_duel', name: 'Final duel', desc: '마지막 결전을 앞둔 순간' },
  { id: 'interrogation', name: 'Interrogation room', desc: '심문실에서 팽팽한 대치' },
...
export default function MobileCharacterDialog({ open, onClose, value, onConfirm }: MobileCharacterDialogProps) {
  const [characterId, setCharacterId] = useState<string>(value?.characterId || CHARACTERS[0].id);
  ...
  const [voice, setVoice] = useState<'verse'|'alloy'|'sage'>(value?.voice || 'verse');
        ...
        <motion.div className="fixed inset-0 z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          >
            ...
            <div className="p-4 space-y-6 overflow-y-auto h-[calc(100%-7rem)]">
              {/* 캐릭터 선택 */}
              <div>
                <div className="text-sm font-medium text-gray-900 mb-2">캐릭터</div>
                  ...
                  {CHARACTERS.map(c => {
                    ...
                    return (
                      <button key={c.id} onClick={() => {
                        setCharacterId(c.id);
                        const meta = CHARACTER_LIST.find(x => x.id === c.id);
                      ...
                      >
                        ...
                        <div className="text-[10px] leading-tight text-center px-1 truncate">{c.name}</div>
                      </button>
                    );
                  })}
                </div>
              ...
              {/* 선택 캐릭터 성격/배경 미리보기 */}
              <div>
                <div className="text-sm font-medium text-gray-900 mb-2">성격/배경</div>
                <div className="text-xs text-gray-600 whitespace-pre-wrap">
                  {(() => {
                    const sel = CHARACTER_LIST.find(c => c.id === characterId);
              ...
              {/* 성별 */}
              <div>
                <div className="text-sm font-medium text-gray-900 mb-2">성별</div>
                <div className="flex items-center space-x-3">
                  {(['male','female'] as GenderOption[]).map(g => (
                    <label key={g} className={`inline-flex items-center space-x-2 px-3 py-2 rounded border ${gender === g ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200'}`}>
              ...
              {/* 목소리 선택 (3종) */}
              <div>
                <div className="text-sm font-medium text-gray-900 mb-2">목소리</div>
                <div className="flex items-center space-x-3">
                  {(['verse','alloy','sage'] as const).map(v => (
                    <label key={v} className={`inline-flex items-center space-x-2 px-3 py-2 rounded border ${voice === v ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200'}`}>
                      <input type="radio" name="voice" checked={voice === v} onChange={() => setVoice(v)} />
            ...
            <div className="border-t px-4 py-3 flex justify-end space-x-2">
              <button onClick={onClose} className="px-4 py-2 text-sm rounded border border-gray-300 text-gray-700 bg-white">취소</button>
              <button onClick={confirm} className="px-4 py-2 text-sm rounded bg-indigo-600 text-white">확인</button>
            </div>
          </motion.div>
        </motion.div>
      )}
