...
export default function MobileChat() {
  ...
  const buildPersonaInstructions = () => {
    ...
    const languageNote =
      "Respond primarily in English. Keep replies to 1–3 sentences of natural English, model better phrasing implicitly, and ask one brief follow-up. If the user specifically requests Korean (한국어로 답변해줘, 한글로 말해줘, etc.), then respond in Korean using formal, respectful language. ";
  ...
  const startVoice = async () => {
    ...
    try {
      ...
      if (conn.dc && conn.dc.readyState === "open") {
        ...
        try {
          conn.dc?.addEventListener("open", sendGreeting as any);
        } catch {}
      }
    } catch (e) {
